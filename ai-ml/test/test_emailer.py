...
Tests all behaviours specified in the implementation plan:
  - dry_run mode returns correct SendResult without hitting SMTP
  - body_preview is first 200 chars of body
  - all required attributes present in dry_run response
  - SMTP exception is caught and returned as error SendResult (not raised)
  - send_email signature matches spec: send_email(to, subject, body) -> SendResult
  - SendResult.success is True for dry_run/sent, False for error
"""
...
from src.emailer import SendResult
...
# ── Test 2: dry_run mode — correct status and attributes ─────────────────────
print("[2] DRY_RUN=True returns correct SendResult without SMTP call")
...
assert isinstance(result, SendResult), f"Expected SendResult, got: {type(result)}"
assert result.status == "dry_run", f"Expected dry_run, got: {result.status}"
assert result.success is True, f"Expected success=True for dry_run, got: {result.success}"
assert result.error is None, f"Expected error=None for dry_run, got: {result.error}"
assert result.to == "billing@acmecorp.com"
assert result.timestamp
assert result.body_preview
print("    PASS  status='dry_run'")
print("    PASS  success=True")
print("    PASS  error=None")
print("    PASS  'to' matches recipient")
print("    PASS  'timestamp' present")
print("    PASS  'body_preview' present")
print()
...
# ── Test 4: SMTP exception caught — returns error SendResult, never raises ────
print("[4] SMTP error is caught and returned as error SendResult — never raised")
...
assert isinstance(err_result, SendResult), f"Expected SendResult, got: {type(err_result)}"
assert err_result.status == "error", f"Expected error, got: {err_result.status}"
assert err_result.success is False, f"Expected success=False, got: {err_result.success}"
assert err_result.error is not None, "Missing error string in error SendResult"
assert "Authentication failed" in err_result.error
assert err_result.timestamp
print("    PASS  SMTP exception caught — status='error'")
print(f"    PASS  error string present: '{err_result.error}'")
print("    PASS  success=False")
...
assert sent_result.success is True, f"Expected success=True, got: {sent_result.success}"
assert sent_result.error is None, f"Expected error=None, got: {sent_result.error}"
assert sent_result.timestamp
