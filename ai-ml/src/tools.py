...
def process_invoice(invoice_no: str) -> str:
    ...
    # Guard 1 — legal_escalation (Stage 5): HALT before any LLM/email work
    from src.triage import TIER_LEGAL
    if urgency_tier == TIER_LEGAL:
        msg = (
            f"HALTED invoice_id={invoice_no} | >30 days overdue (Stage 5). "
            f"Manual legal/finance review required — no email sent."
        )
        logger.log_action(invoice_no, "stage5_halt", "HALTED", msg)
        return json.dumps({
            "status": "HALTED",
            "invoice_no": invoice_no,
            "reason": "Stage 5: Manual Legal/Finance review required. No email dispatched.",
        })

    # Guard 2 — n/a means the invoice is paid or not yet actionable
    if urgency_tier == "n/a":
